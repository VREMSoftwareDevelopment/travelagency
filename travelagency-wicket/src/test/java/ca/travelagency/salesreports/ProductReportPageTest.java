...
public class ProductReportPageTest extends BaseWicketTester {
	...
	public void testProductPage() throws Exception {
		ProductSalesModel model = ProductSalesModel.make(salesSearch);

		tester.startPage(new ProductReportPage(model));
		tester.assertRenderedPage(ProductReportPage.class);
		tester.assertModelValue(BasePrintPage.TITLE, ProductReportPage.REPORT_TITLE);
...
//		System.out.println(lastResponseAsString);
		...
		String expected = velocityTemplateHelper.getExpected(ProductReportPageTest.class.getResourceAsStream("expected-product-report.html"));
